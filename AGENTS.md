...
- `ScheduleView.vue` — 定时任务总览和管理入口。“即将运行”展开未来 24 小时内最多 50 个预计触发实例，同一 cron 可重复出现并按计划运行时间升序排列；“今日运行 / 已停止 / 全部”仍按定时规则每条一行展示；支持新增、编辑、启用/禁用和删除。下方合并展示具有定时规则的脚本/工作流最近 50 条顶层运行，默认同时显示手动与定时来源，并提供“全部 / 仅看定时 / 仅看手动”筛选；脚本记录直接查看历史日志，工作流记录按节点展示状态、耗时和对应脚本日志，窄窗口使用节点下拉选择器。工作流节点记录不会重复作为脚本顶层运行显示。
