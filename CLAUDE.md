...
- `ScheduleView.vue` — 定时任务总览和管理入口。任务列表默认展示即将运行的启用任务，并按下次运行时间升序排列；提供“即将运行 / 今日运行 / 已停止 / 全部”筛选；支持新增、编辑、启用/禁用和删除；下方合并展示脚本/工作流最近运行情况，脚本记录直接查看历史日志，工作流记录按节点展示状态、耗时和对应脚本日志，窄窗口使用节点下拉选择器。脚本用蓝色竖线标识，工作流用橙色竖线标识。
